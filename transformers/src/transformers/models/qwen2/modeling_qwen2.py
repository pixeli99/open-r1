...
import random  # added for random loop support
...
class Qwen2Model(Qwen2PreTrainedModel):
    ...
    def forward(
        ...
        **flash_attn_kwargs: Unpack[FlashAttentionKwargs],
        ...
        # --- Random‑loop support start ---
        # If `config.loop_k_max` > 1, each decoder layer is randomly
        # unrolled 1‒loop_k_max times during *training* to simulate
        # deeper reasoning. Setting it to 1 (default) disables looping.
        loop_k_max = getattr(self.config, "loop_k_max", 1)  # 1 ⇒ no loop (default)

        for layer_idx, decoder_layer in enumerate(self.layers[: self.config.num_hidden_layers]):
            if output_hidden_states:
                all_hidden_states += (hidden_states,)

            # decide how many times to unroll this layer
            if 20 < layer_idx < 26:
                k_steps = random.randint(1, loop_k_max) if (self.training and loop_k_max > 1) else 1
            else:
                k_steps = 1

            for k in range(k_steps):
                # Disable kv‑cache for inner iterations to avoid duplicating keys/values
                inner_use_cache = use_cache if k == 0 else False
                inner_past_kv = past_key_values if (k == 0) else None

                layer_outputs = decoder_layer(
                    hidden_states,
                    attention_mask=causal_mask,
                    position_ids=position_ids,
                    past_key_value=inner_past_kv,
                    output_attentions=output_attentions,
                    use_cache=inner_use_cache,
                    cache_position=cache_position,
                    position_embeddings=position_embeddings,
                    **flash_attn_kwargs,
                )
            ...
            # record attentions from the **last** unroll only
            if output_attentions:
                all_self_attns += (layer_outputs[1],)
        # --- Random‑loop support end ---
